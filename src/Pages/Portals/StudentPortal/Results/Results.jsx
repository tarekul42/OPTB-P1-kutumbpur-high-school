  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-gradient-to-br from-primary-900 to-primary-800 rounded-2xl p-6 text-white shadow-lg md:col-span-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-primary-200 text-sm">{student.class} | Session 2025-26</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-md">
          <h3 className="text-lg font-serif font-bold text-primary-900 mb-4">Term-wise Performance</h3>
          <div className="h-80">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-md">
          <h3 className="text-lg font-serif font-bold text-primary-900 mb-4">Total Marks by Subject</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={combinedMarks} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                <XAxis type="number" domain={[0, 200]} tick={{ fontSize: 12 }} stroke="#6b7280" />
                <YAxis dataKey="subject" type="category" tick={{ fontSize: 12 }} stroke="#6b7280" width={80} />
                <Tooltip content={<CustomTooltip />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-md lg:col-span-2">
          <h3 className="text-lg font-serif font-bold text-primary-900 mb-4">Subject-wise Progress Trend</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={combinedMarks}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Legend />
                <Line type="monotone" dataKey="First Term" stroke="#102a43" strokeWidth={2} dot={{ fill: "#102a43", r: 4 }} />
                <Line type="monotone" dataKey="Second Term" stroke="#d4af37" strokeWidth={2} dot={{ fill: "#d4af37", r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {academicResults.map((result, index) => {
                ...
                if (result.total >= 180) { grade = "A+"; gradeColor = "text-accent-600 bg-accent-100"; }
                else if (result.total >= 160) { grade = "A"; gradeColor = "text-accent-500 bg-accent-50"; }
                else if (result.total >= 140) { grade = "B"; gradeColor = "text-primary-600 bg-primary-100"; }
                else if (result.total >= 120) { grade = "C"; gradeColor = "text-amber-600 bg-amber-100"; }
                else { grade = "D"; gradeColor = "text-red-600 bg-red-100"; }
                ...
                return (
                  <tr key={index} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="text-center py-3 px-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-bold ${gradeColor}`}>{grade}</span>
                    </td>
                  </tr>
                );
              })}
