import { QuarterlyFee, MonthlyFee, FeeSummary, ChildFees } from "@/shared/types/user";
...
export const generateFeesForChild = (childId: number, year = 2026): ChildFees => {
  ...
  const fees: QuarterlyFee[] = quarters.map((q, quarterIndex) => {
    ...
    const startMonth = (q.monthsNum && q.monthsNum[0]) || 1;
    const dueDate = new Date(year, startMonth - 1, 15);
    const paidDate = isPaid ? new Date(year, startMonth + Math.floor(Math.random() * 2), Math.floor(Math.random() * 15) + 1) : null;
